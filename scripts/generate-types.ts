...
const OUT_OPERATION_CLASSIFICATION = path.resolve(
  process.cwd(),
...
const OUT_KNOWN_TOP_LEVEL_OBJECTS = path.resolve(
  process.cwd(),
...
async function main(): Promise<void> {
  ...
  const mutationObjectsByVersion = stableRecord(
    versions.map(version => {
      const artifact = artifactsByVersion[version];
      if (!artifact) throw new Error(`Missing generated artifacts for ${version}`);
      return [
        ...
        [
          ...new Set(
            Object.values(artifact.operationMetadata)
              .filter(
                metadata =>
                  Boolean(metadata.object) &&
                  !(
                    metadata.kind === 'crud' &&
                    (metadata.verb === 'get' || metadata.verb === 'list')
                  )
              )
              .map(metadata => metadata.object!)
          ),
        ].sort(),
      ] as const;
    })
  ...
  const knownTopLevelObjects = [
    ...new Set(versions.flatMap(version => artifactsByVersion[version]!.objects)),
  ].sort();
...
`;
  ...
  const operationClassificationFile = await format(
    `// AUTO-GENERATED by scripts/generate-types.ts. Do not edit by hand.
...
export const AXL_READ_ONLY_OPERATIONS_BY_VERSION: Record<WsdlVersion, readonly string[]> = ${JSON.stringify(readOnlyOperationsByVersion, null, 2)};
...
export const AXL_MUTATION_OBJECTS_BY_VERSION: Record<WsdlVersion, readonly string[]> = ${JSON.stringify(mutationObjectsByVersion, null, 2)};
`,
    {
      parser: 'typescript',
      singleQuote: true,
...
`;
  ...
  const knownTopLevelObjectsFile = `/* eslint-disable */
// AUTO-GENERATED by scripts/generate-types.ts. Do not edit by hand.
// Union of object names across all supported WSDL versions. This intentionally
// avoids importing a version payload at MCP startup.

export const ALL_KNOWN_AXL_TOP_LEVEL_OBJECTS = ${JSON.stringify(knownTopLevelObjects)} as const;
`;
...
`;
  ...
  await Promise.all([
    ...
    fs.mkdir(path.dirname(OUT_OPERATION_CLASSIFICATION), { recursive: true }),
    fs.mkdir(path.dirname(OUT_KNOWN_TOP_LEVEL_OBJECTS), { recursive: true }),
  ...
  await Promise.all([
    ...
    fs.writeFile(OUT_KNOWN_TOP_LEVEL_OBJECTS, knownTopLevelObjectsFile, 'utf8'),
    fs.writeFile(OUT_LATEST, latestFile, 'utf8'),
    fs.writeFile(OUT_AXL_OBJECTS, axlObjectsFile, 'utf8'),
    fs.writeFile(OUT_OPERATION_CLASSIFICATION, operationClassificationFile, 'utf8'),
