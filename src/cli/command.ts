...
function parseArguments(
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (!field || !allowed.has(field)) usage(`Unknown option "${token}" for ${command}`);
    if (Object.prototype.hasOwnProperty.call(flags, field))
      usage(`Option "${token}" was provided twice`);

    if (valueField) {
      const value = tokens[index + 1];
      if (value === undefined || value.startsWith('--'))
        usage(`Option "${token}" requires a value`);
      flags[field] = value;
      index++;
    } else {
      flags[field] = true;
...
export function parseCliCommand(argv: readonly string[]): CliCommand {
  ...
  if (!['versions', 'objects', 'operations', 'describe', 'execute', 'sql'].includes(commandToken)) {
    usage(`Unknown command "${commandToken}"`);
  }
